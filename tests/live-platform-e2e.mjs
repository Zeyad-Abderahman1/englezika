...
async function call(path, { cookie, json, body, headers = {}, method = "GET" } = {}) {
  ...
  const response = await fetch(`${base}${path}`, { method, headers: requestHeaders, body: payload, redirect: "manual" });
...
const enrollmentRow = bootstrap.result.enrollments.find((item) => item.userEmail === studentEmail && item.courseId === courseId);
...
const createGrader = await call("/api/admin/staff", {
  method: "POST",
  cookie: teacherLogin.cookie,
  json: { name: "Grade Assistant", email: graderEmail, password: "GradeAssistant!2026", role: "assistant", preset: "grader" },
});
...
const createManager = await call("/api/admin/staff", {
  method: "POST",
  cookie: teacherLogin.cookie,
  json: { name: "Course Assistant", email: managerEmail, password: "CourseManager!2026", role: "assistant", preset: "course_manager" },
});
