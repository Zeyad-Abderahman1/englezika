...
function parsePermissions(value: string): StaffPermission[] {
  try {
    ...
    return parsed.filter((item): item is StaffPermission =>
...
export async function hashPassword(password: string, saltHex?: string, iterations = PASSWORD_ITERATIONS) {
  const salt = saltHex
...
export async function verifyStaffCredentials(emailInput: string, password: string): Promise<StaffSession | null> {
  ...
  if (!constantTimeEqual(candidate.hash, row.passwordHash)) {
    ...
    ).bind(failures, lockedUntil, Date.now(), email).run();
    return null;
  }
  await db.prepare(
...
export async function createStaffSession(staff: StaffSession): Promise<{ token: string; expiresAt: number }> {
...
export async function getCurrentStaff(request?: Request): Promise<StaffSession | null> {
  ...
  ).bind(tokenHash, now).first<{ expiresAt: number; email: string; name: string; role: string; permissions: string }>();
  if (!row) return null;
  await getD1().prepare("UPDATE staff_sessions SET last_seen = ? WHERE token_hash = ?").bind(now, tokenHash).run();
  return {
    email: row.email,
    name: row.name,
    role: row.role === "teacher" ? "teacher" : "assistant",
...
export async function deleteCurrentStaffSession(request: Request): Promise<void> {
  ...
  if (!token) return;
  await ensureDatabase();
  await getD1().prepare("DELETE FROM staff_sessions WHERE token_hash = ?").bind(await sha256(token)).run();
}

export async function apiStaff(request: Request, permission?: StaffPermission): Promise<StaffSession | Response> {
