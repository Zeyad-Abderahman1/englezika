...
type QuestionRow = {
  id: string; type: string; prompt: string; options: string | null;
  correctAnswer: string; rubric: string; points: number; sortOrder: number;
};
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (exam.closesAt && Number(exam.closesAt) < now) return jsonError("انتهى وقت إتاحة الامتحان", 403);
  const email = user.email.toLowerCase();
  const attemptCount = await getD1().prepare(
    "SELECT COUNT(*) AS count FROM attempts WHERE exam_id = ? AND user_email = ?",
  ).bind(id, email).first<{ count: number }>();
  ...
  ).bind(sessionId, id, email, startedAt, expiresAt).run();
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const session = await getD1().prepare(
    "SELECT id, started_at AS startedAt, expires_at AS expiresAt, status FROM exam_sessions WHERE exam_id = ? AND user_email = ?"
  ).bind(id, email).first<{ id: string; startedAt: number; expiresAt: number; status: string }>();
  if (!session || session.status !== "active") return jsonError("جلسة الامتحان غير صالحة، افتح الامتحان من حسابك", 409);
  ...
  const questionResult = await getD1().prepare(
  ...
  ).bind(id).all<QuestionRow>();
  if (!questionResult.results.length) return jsonError("لا توجد أسئلة في هذا الامتحان", 409);

  const objective: Array<{ question: QuestionRow; answer: string; score: number; feedback: string }> = [];
  ...
  for (const question of questionResult.results) {
    ...
    if (question.type === "short_answer") {
      ...
      const correct = answer.trim().toLocaleLowerCase("en") === question.correctAnswer.trim().toLocaleLowerCase("en");
      objective.push({
        question,
        answer,
  ...
  const feedback = percentage >= Number(exam.passingScore || 50)
  ...
  await db.batch([
    ...
    ...allAnswers.map((item) => db.prepare(
      "INSERT INTO answers (id, attempt_id, question_id, answer, score, feedback) VALUES (?, ?, ?, ?, ?, ?)"
    ).bind(crypto.randomUUID(), attemptId, item.question.id, item.answer, item.score, item.feedback)),
