...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await getD1().prepare(
    `UPDATE courses SET title = ?, grade = ?, description = ?, price = ?, status = ?, updated_at = ?
     WHERE id = ?`
  ).bind(title, grade, description, price, status, Date.now(), id).run();
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (Number(dependencies?.count)) return jsonError("لا يمكن حذف كورس مرتبط بطلاب أو امتحانات أو فيديوهات", 409);
