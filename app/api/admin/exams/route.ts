...
function parseQuestion(question: RawQuestion, sortOrder: number) {
  ...
  const options = Array.isArray(question.options)
    ? question.options.map((option) => safeText(option, 300)).filter(Boolean).slice(0, 8)
    : [];
...
export async function POST(request: Request) {
  ...
  const questions = Array.isArray(body.questions)
    ? body.questions.slice(0, 100).map((question, index) => parseQuestion(question as RawQuestion, index + 1))
    : [];
  if (title.length < 3) return jsonError("اسم الامتحان مطلوب");
  if (!questions.length || questions.some((question) => !question.prompt || !question.correctAnswer)) {
    return jsonError("أضف سؤالاً واحداً على الأقل مع الإجابة الصحيحة");
  }
  if (questions.some((question) => question.type === "multiple_choice" && question.options.length < 2)) {
  ...
  await db.batch([
    db.prepare(
      ...
      id, courseId, title, description, instructions, durationMinutes, passingScore, maxAttempts, status,
      body.opensAt ? Number(body.opensAt) : null,
      body.closesAt ? Number(body.closesAt) : null,
      admin.email.toLowerCase(), now, now,
    ),
    ...questions.map((question) => db.prepare(
      `INSERT INTO questions
       (id, exam_id, sort_order, type, prompt, options, correct_answer, rubric, points)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      crypto.randomUUID(), id, question.sortOrder, question.type, question.prompt,
      JSON.stringify(question.options), question.correctAnswer, question.rubric, question.points,
    )),
  ]);
