...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.prepare(
    "SELECT id, course_id AS courseId FROM videos WHERE id = ?"
  ).bind(id).first<{ id: string; courseId: string }>();
  ...
  if (prerequisiteExamId) {
    const exam = await db.prepare(
      "SELECT id FROM exams WHERE id = ? AND course_id = ? AND status = 'published'",
    ).bind(prerequisiteExamId, existing.courseId).first();
  ...
  ).bind(title, prerequisiteExamId, minimumScore, status, id).run();
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const db = getD1();
  const video = await db.prepare("SELECT r2_key AS r2Key FROM videos WHERE id = ?")
