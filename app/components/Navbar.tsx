      ...
      <nav className="nav-shell" aria-label="التنقل الرئيسي">
        ...
        <div className={`nav-menu ${open ? "is-open" : ""}`}>
          {publicLinks.map((link) => (
            <Link key={link.href} href={link.href} className={pathname === link.href ? "active" : ""} onClick={closeMenu}>{link.label}</Link>
          ))}
          {viewer && <Link href="/account" className={pathname === "/account" ? "active" : ""} onClick={closeMenu}>مساحتي التعليمية</Link>}
        </div>

        <div className="nav-actions">
          <button className={`theme-toggle ${light ? "is-light" : "is-dark"}`} onClick={toggleTheme} aria-pressed={light} aria-label={light ? "تفعيل الوضع الداكن" : "تفعيل الوضع الفاتح"}>
            <span className="theme-thumb" aria-hidden="true" />
            <span className="theme-icon theme-moon" aria-hidden="true"><MoonStar size={16} /></span>
            <span className="theme-icon theme-sun" aria-hidden="true"><Sun size={17} /></span>
          </button>
          {viewer ? (
            <>
              ...
              <a href="/student/logout" className="btn btn-primary nav-register">تسجيل الخروج</a>
            </>
          ) : (
            <>
              <Link href="/login" className="btn btn-ghost nav-login">تسجيل الدخول</Link>
              <Link href="/register" className="btn btn-primary nav-register">حساب جديد</Link>
            </>
          )}
