      ...
      <section className="secure-player-card">
        {active?.unlocked ? (
          ...
          <div className="locked-lesson">
            ...
            <p>
              أكمل {active?.prerequisiteExamTitle || 'اختبار المحاضرة'}
              {active?.minimumScore ? ` واحصل على ${active.minimumScore}% على الأقل` : ''} قبل تشغيل
              الفيديو.
            </p>
            {active?.prerequisiteExamId && (
              <Link href={`/exam/${active.prerequisiteExamId}`} className="btn btn-primary">
                <FileQuestion /> ابدأ الاختبار
              </Link>
            )}
          </div>
        )}
        ...
        {active?.unlocked && (
          <div className="video-security-note">
            <LockKeyhole /> الفيديو مرتبط بحسابك ويُعرض بعلامة مائية. مشاركة الحساب أو تسجيل المحتوى
            مخالفة لشروط المنصة.
          </div>
        )}
      </section>
        ...
        <div>
          {videos.map((video, index) => (
            ...
            >
              ...
              {video.unlocked ? <PlayCircle /> : <LockKeyhole />}
            </button>
          ))}
        </div>
      </aside>
