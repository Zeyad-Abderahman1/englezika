...
export default function PaymentFlow({ courseId }: { courseId: string }) {
  ...
  if (done) return (
    <div className="payment-success">
      <CheckCircle2 /><h2>طلبك وصل يا بطل!</h2>
      <p>الإدارة هتراجع بيانات الدفع وتفعّل الكورس على حسابك. تقدر تتابع الحالة من مساحتك التعليمية.</p>
      <Link href="/account" className="btn btn-primary">افتح حسابي</Link>
    </div>
  );
  return (
    <div className="payment-card">
      ...
      <label>رقم العملية أو التحويل
        <input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="اكتب الرقم المرجعي إن وجد" />
      </label>
      <button
        onClick={async () => {
          setLoading(true);
          ...
          const response = await fetch("/api/enrollments", {
            ...
            body: JSON.stringify({ courseId, paymentMethod: selected, paymentReference: reference }),
          });
      ...
      <p className="form-hint">لن يتم تخزين بيانات البطاقة على المنصة. الطلب يحتاج مراجعة الإدارة قبل فتح المحتوى.</p>
