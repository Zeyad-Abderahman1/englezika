...
export default function SubscribeClient({ courseId }: { courseId: string }) {
  ...
  if (!course) return <div className="dashboard-state"><LoaderCircle className="spin" /> جاري تحميل بيانات الاشتراك...</div>;
  return (
    <div className="container subscribe-grid">
      <aside className="order-summary"><span>ملخص الطلب</span><h2>{course.month}</h2><p>{course.grade} · {course.lectures || "عدة"} محاضرات</p><div><span>الإجمالي</span><strong>{course.price} جنيه</strong></div></aside>
